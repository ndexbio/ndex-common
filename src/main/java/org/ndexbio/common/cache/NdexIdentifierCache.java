...
public enum NdexIdentifierCache {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(NdexIdentifierCache.class);
	private static final Long CACHE_SIZE = 4000000L;

	private LoadingCache<String, Long> identifierCache;
	/*
	 * the key for a BaseTern entry is ns:value the key for a FunctionTerm entry
	 * is a concatenation of its children's id the value is the JdexId for the
	 * term itself
	 */
	private LoadingCache<String, Long> termCache;

	public boolean isNovelTerm(String aTermIdentifier) {
		if (Strings.isNullOrEmpty(aTermIdentifier)) {
			return false;
		}
	...
	private RemovalListener<String, Long> identifierListener = new RemovalListener<String, Long>() {
		...
		public void onRemoval(RemovalNotification<String, Long> removal) {
			if (!removal.getCause().toString().equalsIgnoreCase("EXPLICIT")) {
				logger.error("*****Identifier removed from cache key= "
						+ removal.getKey() + " Cause= "
	...
	// Identifier cache
	private RemovalListener<String, Long> termListener = new RemovalListener<String, Long>() {

		public void onRemoval(RemovalNotification<String, Long> removal) {
			if (!removal.getCause().toString().equalsIgnoreCase("EXPLICIT")) {
				logger.error("*****Term removed from cache key= "
						+ removal.getKey() + " Cause= "
						+ removal.getCause().toString());
			}
		}
	...
	private void initializeTermCache() {
		this.termCache = CacheBuilder.newBuilder().maximumSize(CACHE_SIZE)
				.expireAfterAccess(180L, TimeUnit.MINUTES)
				.removalListener(termListener)
				.build(new CacheLoader<String, Long>() {
					@Override
