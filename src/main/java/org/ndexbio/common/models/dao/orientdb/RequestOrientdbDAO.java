	...
	/*		final List<ODocument> existingRequests = _ndexDatabase
			...
			if (existingRequests == null || existingRequests.isEmpty())
				...
				throw new DuplicateObjectException(
						"You have already made that request and cannot make another.");
...
/*			final IRequest requestToUpdate = _orientDbGraph.getVertex(
			...
			if (!updatedRequest.getResponse().equals("DECLINED")) {
				...
				else if (updatedRequest.getRequestType().equals(
						"Network Access"))
...
/*		final IGroup requestingGroup = _orientDbGraph.getVertex(fromRid,
		...
		final IUser requestedUser = _orientDbGraph
				.getVertex(toRid, IUser.class);
		if (requestedUser == null)
			throw new ObjectNotFoundException("User", toRid);

		final IGroupInvitationRequest newRequest = _orientDbGraph.addVertex(
				"class:groupInvite", IGroupInvitationRequest.class);
	...
	/*	final IUser requestOwner = _orientDbGraph.getVertex(fromRid,
		...
		final IGroup requestedGroup = _orientDbGraph.getVertex(toRid,
				IGroup.class);
		if (requestedGroup == null)
			throw new ObjectNotFoundException("Group", toRid);

		final IJoinGroupRequest newRequest = _orientDbGraph.addVertex(
				"class:joinGroup", IJoinGroupRequest.class);
		newRequest.setMessage(requestToCreate.getMessage());
		newRequest.setRequestTime(new Date());
...
/*		final IUser requestOwner = _orientDbGraph.getVertex(fromRid,
		...
		final INetwork requestedNetwork = _orientDbGraph.getVertex(toRid,
				INetwork.class);
		if (requestedNetwork == null)
			throw new ObjectNotFoundException("Network", toRid);

		final INetworkAccessRequest newRequest = _orientDbGraph.addVertex(
				"class:networkAccess", INetworkAccessRequest.class);
		newRequest.setMessage(requestToCreate.getMessage());
		newRequest.setRequestTime(new Date());
		newRequest.setFromUser(requestOwner);
...
//		IUser user = _orientDbGraph.getVertex(userId, IUser.class);
		...
		newMember.setPermissions(Permissions.valueOf(requestToProcess
				.getResponse()));
