...
/******************************************************************************
 * Converts between RIDs and JIDs. JIDs are used on the web site as a workaround
 * for the limitations of limited characters that can be displayed in URLs. RIDs
 * are the true IDs used by OrientDB.
 ******************************************************************************/
public class IdConverter {
	...
	public static ORID toRid(String Jid) throws IllegalArgumentException {
		final Matcher m = Pattern.compile("^C(\\d*)R(\\d*)$").matcher(
				Jid.trim());
	...
	public static String toRidCsv(String[] strings) {
		String resultString = "";
		for (final String string : strings) {
			ORID rid = toRid(string);
			resultString += rid + ",";
		}
